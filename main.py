...
def post_to_bs(text, user, password):
    ...
    except Exception as e: print(f"❌ BS Error ({user}): {e}")
    ...
    for feed in FEEDS:
        ...
        for entry in parsed.entries:
            if entry.link not in posted_links:
                ...
                prompt = f"Write a viral social media post in Arabic about: {entry.title}. Link: {entry.link}. Use tech hashtags."
